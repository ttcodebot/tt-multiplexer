...
class Polygon:
	...
	def _edge_touch(self, points1, points2):
		# Create edges list
		edge_fn = lambda n: [(x,(x+1)%n) for x in range(n)]

		edges1 = points1[edge_fn(len(points1))]
		edges2 = points2[edge_fn(len(points2))]
		...
		for e1 in edges1:
			...
			for e2 in edges2:
				# If not horizontal, and on same edge, skip
				if (e2[0][ci^1] != ref) or (e2[1][ci^1] != ref):
					continue
	...
	def intersect(self, other):
		...
		pt_self  = self.gp.points.astype(np.float32)
		pt_other = other.gp.points.astype(np.float32)

		if other.gp.contain_any(*pt_self) and not self.gp.contain_any(*pt_other):
			return True

		# Nope, try the intersection
		if gdstk.boolean(self.gp, other.gp, 'and'):
			return True

		# Last chance, edges touchings
		if self._edge_touch(pt_self, pt_other):
			return True
...
class NetlistExtractor:
	...
	def write_spice(self, filename):
		...
		for i,ref in enumerate(self.top.references):
			args_conn = ' '.join([f'net_{self.pads[(ref,n)][0].net}' for n in pl.get(ref.cell.name,[])])
